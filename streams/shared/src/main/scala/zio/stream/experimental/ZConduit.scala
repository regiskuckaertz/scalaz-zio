...
object Signal {
  case object End extends Signal[Nothing] {
    type Emit[+O]          = Nothing
  ...
  final case class Continue[I](input: Chunk[I]) extends Signal[I] {
    type Emit[+O]          = Chunk[O]
...
sealed abstract class ZConduit[-R, +E, -I, +O, +Z] private[stream] (
  val run: ZManaged[R, Nothing, SignalFunction[R, E, I, O, Z]]
) extends Serializable

// sealed abstract class ZStream[-R, +E, +O](
//   override val run: ZManaged[R, Nothing, SignalFunction[R, E, Unit, O, Unit]]
// ) extends ZConduit[R, E, Unit, O, Unit](run) { self =>
// import ZStream.Pull
...
sealed abstract class ZSink[-R, +E, -I, +Z](
  override val run: ZManaged[R, Nothing, SignalFunction[R, E, I, Unit, Z]]
) extends ZConduit[R, E, I, Unit, Z](run)

object ZSink {
  object Push {
    def emit[A](a: A): UIO[A]  = IO.succeedNow(a)
    val next: UIO[Chunk[Unit]] = UIO(Chunk.unit)
  }

  def apply[R, E, I, Z](run: ZManaged[R, Nothing, SignalFunction[R, E, I, Unit, Z]]): ZSink[R, E, I, Z] =
    new ZSink(run) {}
  ...
  //   ZSink(Managed.succeedNow(_ => Push.next))
}

// sealed abstract class ZTransducer[-R, +E, -I, +O](
//   override val run: ZManaged[R, Nothing, SignalFunction[R, E, I, O, Nothing]]
